...
class LexicalAnalyzer
{
    ...
    static void ProcessToken(string token, List<string> tokens, List<string[]> symbolTable)
    {
        ...
        else if (variable_Reg.IsMatch(token))
        {
            ...
            if (!symbolTable.Exists(entry => entry[1] == token))
            {
                // Assuming "int" as the default type for simplicity
                symbolTable.Add(new string[] { symbolTable.Count.ToString(), token, "int", "Unknown" });
            }
        }
        else if (constants_Reg.IsMatch(token))
        {
            ...
            if (!symbolTable.Exists(entry => entry[1] == token))
            {
                symbolTable.Add(new string[] { symbolTable.Count.ToString(), token, type, token });
            }
        }
